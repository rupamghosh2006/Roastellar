          ...
          {mobileNavItems.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                className={cn(
                  'flex flex-col items-center justify-center rounded-2xl px-1 py-2 text-center transition-colors',
                  isActive ? 'bg-white/10 text-white' : 'text-white/55 hover:bg-white/[0.06] hover:text-white'
                )}
              >
                <Icon className={cn('h-4 w-4', isActive ? 'text-blue-300' : 'text-white/65')} />
